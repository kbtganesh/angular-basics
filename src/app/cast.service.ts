...
export class CastService {
  ...
  addCast (cast: Cast): Observable<Cast> {
    return this.http.post<Cast>(this.heroesUrl, cast, httpOptions)
      .pipe(
  ...
  deleteCast(id: number) {
    ...
    return this.http.delete(url, httpOptions)
      .pipe(
        ...
        catchError(this.handleError<Cast>('deleteHero'))
      );
  }

  searchCastings(term: string): Observable<Cast[]> {
    if (!term.trim()) {
      // if not search term, return empty hero array.
      return of([]);
    }
    return this.http.get<Cast[]>(`${this.heroesUrl}/?name=${term}`).pipe(
      tap(_ => this.log(`found heroes matching "${term}"`)),
      catchError(this.handleError<Cast[]>('searchHeroes', []))
    );
  }
